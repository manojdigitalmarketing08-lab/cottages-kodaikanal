...
(function () {
  ...
  /* ------------------------------------------------------------------ */
  /* Mark JS as running, and reveal any [data-reveal] element already in */
  /* view, in the same tick this class gets added. This class must only  */
  /* ever be set here — once partials have loaded and every page's own   */
  /* reveal-observer script (if any) is about to wire up — never from an */
  /* inline <head> script. Adding it early would hide [data-reveal]      */
  /* content (per each page's CSS) the instant the stylesheet applies,   */
  /* long before this deferred, network-gated script can run to reveal   */
  /* it again — leaving real content invisible on any slow connection.   */
  /* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
  /* Popular Routes carousel — ported from the "exp" reference design's  */
  /* routesTrack logic (dots built dynamically, arrow nav scrolls by     */
  /* one card width) rather than final's scroll-snap-state pattern, so   */
  /* the ported markup/CSS keeps its original interaction intact         */
  /* ------------------------------------------------------------------ */
  const routesTrack = document.getElementById("routesTrack");

  if (routesTrack) {
    const cards = Array.from(routesTrack.querySelectorAll(".route-card-v2"));
    const dotsWrap = document.getElementById("routesDots");
    const prevBtn = document.querySelector(".routes-nav-prev");
    const nextBtn = document.querySelector(".routes-nav-next");

    cards.forEach((_, i) => {
      const dot = document.createElement("button");
      dot.type = "button";
      dot.className = "routes-dot" + (i === 0 ? " active" : "");
      dot.setAttribute("aria-label", `Go to route ${i + 1}`);
      dot.addEventListener("click", () => {
        cards[i].scrollIntoView({ behavior: "smooth", inline: "start", block: "nearest" });
      });
      dotsWrap?.appendChild(dot);
    });
    const routeDots = Array.from(dotsWrap?.querySelectorAll(".routes-dot") ?? []);

    const setActiveRouteDot = () => {
      const trackLeft = routesTrack.getBoundingClientRect().left;
      let closest = 0;
      ...
      routeDots.forEach((d, i) => d.classList.toggle("active", i === closest));
    };

    routesTrack.addEventListener("scroll", () => window.requestAnimationFrame(setActiveRouteDot), { passive: true });

    const scrollRoutesByCard = (direction) => {
      const card = cards[0];
      if (!card) return;
      const gap = 16;
      const amount = (card.offsetWidth + gap) * direction;
      routesTrack.scrollBy({ left: amount, behavior: "smooth" });
    };

    prevBtn?.addEventListener("click", () => scrollRoutesByCard(-1));
    nextBtn?.addEventListener("click", () => scrollRoutesByCard(1));
  ...
  const propertiesSliderWrapper = document.getElementById("properties-slider-wrapper");
  const propertiesEmptyState = document.getElementById("properties-empty-state");

  // Picking a category only updates which category is "armed" (tab
  // highlight + hidden form field) — it does not reveal any property
  // cards. Cards (images + Book Now/Call) only appear once the hero
  // search form is submitted, via revealProperties() below.
  ...
  function revealProperties(category) {
    if (!category || !propertiesTrack) return;

    propertiesTrack.querySelectorAll(".property-card").forEach((card) => {
      card.hidden = card.dataset.propertyCategory !== category;
    });
    if (propertiesSliderWrapper) propertiesSliderWrapper.hidden = false;
    if (propertiesEmptyState) propertiesEmptyState.hidden = true;
  ...
  propertiesFilter?.addEventListener("click", (event) => {
    ...
    revealProperties(chip.dataset.propertyCategory);
  });
  ...
  if (heroSearch && allPropertiesSection) {
    heroSearch.addEventListener("submit", (event) => {
      event.preventDefault();
      revealProperties(heroCategoryInput?.value || "Hotels");
  ...
  /* ------------------------------------------------------------------ */
  /* Site nav bar — visible only while the hero is in view; hidden on    */
  /* scroll through every other section, on every page                  */
  /* ------------------------------------------------------------------ */
